#!/usr/bin/env bash
set -euo pipefail
SRC=${1:-figures/drawio/exports/system.xml}
OUT_DIR=${2:-figures/drawio/exports}
DRAWIO_CLI=${DRAWIO_CLI:-"npx --yes @drawio/cli"}
mkdir -p "$OUT_DIR"
if [[ ! -f "$SRC" ]]; then
  echo "Draw.io source not found: $SRC" >&2
  echo "Source-of-truth is figures/drawio/exports/system.xml. Edit in Draw.io Desktop and re-export, or provide a custom path as first arg." >&2
  exit 1
fi
