...
\section{Results}
Our analysis of 1,748 Reddit posts and controlled behavioral probes reveals several key findings regarding the "mutual wanting" between users and LLMs. All analyses used real discourse data with no synthetic content.

\textbf{Inter-Annotator Reliability.} Our style clustering annotation achieved strong reliability. Two independent annotators labeled style clusters across 300 posts, achieving Cohen's $\kappa = 0.795$ (95\% CI: 0.72--0.87), indicating good agreement \citep{landis1977measurement}. The confusion matrix (Figure~\ref{fig:confusion}) shows that most disagreements occurred between semantically similar clusters (style\_0 and style\_3), which is expected in subjective annotation tasks.

\textbf{Linguistic Feature Analysis.} Regression analysis revealed significant associations between linguistic features and post engagement (Figure~\ref{fig:forest}). Higher hedge rates were associated with lower scores (OR = 0.97, 95\% CI: 0.96--0.98), while token length showed positive associations (OR = 1.02, 95\% CI: 1.01--1.03). These findings align with prior work on uncertainty communication \citep{kay2016uncertainty} and suggest that the linguistic style of user posts is a significant predictor of community engagement.

\textbf{Behavioral Probes.} Probe analysis comparing high-score versus low-score posts revealed subtle but systematic differences in linguistic patterns (Table~\ref{tab:probes}). While effect sizes were modest, the patterns suggest that post quality is associated with differences in confidence disclosure rates and empathy markers.

\textbf{Temporal Style Drift.} Analysis of style cluster distributions over time revealed systematic shifts (Table~\ref{tab:drift}). The dominant cluster (style\_3) decreased from 68\% to 62\% of posts over the analysis period, while other clusters showed compensatory increases. This suggests an evolution in discourse patterns, possibly in response to the changing behavior of the AI.
