...
def build_from_raw(a_path: str, b_path: str):
    A = load_map(a_path)
    B = load_map(b_path)
...
def main():
    ...
    if args.agreement_json:
        ...
        kappa, n_shared, pairs_counter = build_from_raw(args.a, args.b)
    ...
    print(f"Disagreement report -> {args.out_json} (kappa={kappa:.3f if kappa is not None else 'NA'})")
